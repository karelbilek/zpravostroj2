...
}{Přesnosti a úplnosti neklasifikačních metod}
...
Je zajímavé, že zatímco \uv{opticky} a v~mikroprůměru jsou výsledky stop témat lepší, v~makroprůměru se výsledky zhorší. U tohoto jevu si nejsem schopen vysvětlit příčiny; jako čistou spekulaci mohu uvést to, že 
...
}{Přesnosti a úplnosti klasifikátorů}

Varianty u klasifikátorů jsou shodné s~variantami, definovanými v~části~\ref{sec:klasifikace_priklady}. 

Lze říci, že výsledky dopadají velmi tristně. Vždy dopadá lépe varianta, kdy jako rysy je bráno pouze prvních 10 slov; taktéž je ze všech klasifikátorů nejúspěšnější NaiveBayes (který má také výhodu, že je nejrychlejší).
