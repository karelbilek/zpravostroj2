...
my $month_ = 12;
while ($month_ >= 4) {
	my $url = "http://aktualne.centrum.cz/archiv.phtml?section=1666&limit=20000&date=$day_.$month_.2010&type[1]=clanky";
	my $archive = read_from_web($url);
	
	while ($archive=~/<p> <span><b>(..)\.(..)\.2010<\/b><\/span> <span>\| &nbsp;&nbsp;[^<]*nek<\/span> <span class="vypis-vysledek"><a href="([^"]*)" title="[^"]*">[^<]*<\/a><\/span> <\/p>/g) {
		my $d = $1;
		my $m = $2;
		my $art = $3;
